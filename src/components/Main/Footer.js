...
const Footer = () => {
  ...
  const links = [
    {
      ...
      items: ["درباره ما", "تماس با ما", "محصولات"],
    },
    {
  ...
  return (
    <footer className="bg-pedram-1 relative w-full text-pedram-4 mt-10 md:mt-0">
      <div className="hidden md:inline-flex	justify-between items-center rtl w-full   ">
        {links.map((category) => (
          <div key={category.title} className="text-lg px-32 py-4 w-1/2">
            <div className="text-pedram-3 font-bold">{category.title}</div>
            <div>
              {category.items.map((item) => (
                <div key={item}>
                  <Link href={`/${item}`} className="hover:text-pedram-3">
                    {item}
                  </Link>
                </div>
              ))}
        ...
        <div className="flex items-center space-x-4 rtl p-4 justify-between  md:hidden ">
          {directionIcons.map((item) => (
            <Link key={item.title} href={item.linkTitle}>
              <div className="flex  flex-col items-center  font-bold text-xs  text-pedram-4	 relative group">
                <div className="flex">
      ...
      <div className=" flex justify-between items-center p-4 text-center text-pedram-3">
